...
> Archived 2026-09-22 from `docs/pickpocket-evidence.md`. This is a dated evidence archive. It preserves source clips, live-session
...
The detector now acquires the panel from its header and marker geometry, independently of region count/order/spacing. Live observation, timing telemetry, bounded evidence, and an engine-owned 180-second cooldown are connected; automatic Space delivery is absent. Shuffled synthetic layouts, duplicate colors, cooldown boundaries/restart, and focus loss have regression coverage. The 146-frame replay still has zero labeled mismatches and one hypothetical purple press (latest cropped detector mean 2.263 ms, p95 4.083 ms under concurrent build load). These results do not prove real key-delivery timing. The original plan is preserved in [history/pickpocket-plan-2026-09-03.md](pickpocket-plan-2026-09-03.md); [Pickpocket debugging](../pickpocket-debugging.md) covers current operation.
